import { underscoresNode, createElement, hint, incorrectGuessesCounter, keyboard, gallowsManBlock,
modal, modalMessage, correctWord, playAgainButton } from "../app.js";
...
function generateWord(){
  const wordsArray = [
    ...
    { word: "Кеширование", hint: "Механизм сохранения копии данных или результата вычислений для ускорения будущего доступа." }, 
    { word: "Рефакторинг", hint: "Процесс изменения структуры кода, с целью улучшения его читаемости, понимаемости и поддерживаемости." },
    { word: "Препроцессор", hint: "Инструмент, используемый для расширения функциональности языка стилей CSS, такой как Sass или Less." },
    { word: "Медузы", hint: "Существуют на Земле уже более 500 миллионов лет." },
    { word: "Октопус", hint: "Имеет три сердца и синюю кровь." },
    { word: "Возрождение", hint: "Период в истории Европы, характеризующийся развитием искусства, литературы и науки." },
    ...
    { word: "Полтергейст", hint: "Зловещий дух, известный своей способностью создавать шумы и перемещать предметы." },
    { word: "Призрак", hint: "Невидимое существо, предположительно душа умершего человека." },
    { word: "Экзорцизм", hint: "Обряд изгнания демонов или злых духов из человека или места." },
    { word: "Проклятие", hint: "Магическая формула или действие, навлекающее несчастье или беду." },
    { word: "Апокалипсис", hint: "Конец света" },
    { word: "Ритуал", hint: "Церемония с мистическими действиями, часто связанная с темными силами." },
    { word: "Мистика", hint: "Область, связанная с загадочными и сверхъестественными явлениями." },
  ]
  ...
  console.log(randomNumber, lastRandomNumber, 'after cycle');
...
function createUnderscores(word){
  ...
  for (let i = 0; i < underscoresCount; i += 1){
    const underscore = createElement("span", ["quiz__underscore", "unguessed"], underscoresNode);
  ...
  function controlUnderscores(positionsOfLetters, letter, word){
    ...
    const isThereUnguessedLetter = underscores.some(el => el.classList.contains("unguessed"));
...
function controlIncorrectGuesses(){
  ...
  function countIncorrectGuesses(isQuestionIncorrect = false, word){
    if(isQuestionIncorrect){
      incorrectGuesses+=1;
      gallowsManBlock.childNodes[incorrectGuesses-1].classList.remove("hidden");
...
function startGame(){
  ...
  controlGame(wordsObject, countIncorrectGuesses, controlUnderscores, wordsObject.word);
}

function keyAndVirtualKeyboardListenerWrapper(countIncorrectGuesses, controlUnderscores, word) {
...
function controlGame(wordsObject, countIncorrectGuesses, controlUnderscores){
  ...
  specificListener = keyAndVirtualKeyboardListenerWrapper(countIncorrectGuesses, controlUnderscores, word);
